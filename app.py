...
st.caption("Advanced text mining from academic web sources, PDF documents, and custom inputs.")
st.markdown("---")

with st.sidebar:
    st.header("⚙️ Global Control Panel")
    analysis_mode = st.selectbox("Select Analysis Mode", ["arXiv Web Scraping", "PDF Document Analysis"])
    ...
    else: # PDF Document Analysis
        uploaded_file = st.file_uploader("Upload PDF file", type=["pdf"])
        launch_btn = st.button("Analyze PDF Document")
...
if launch_btn:
    ...
    if df is not None:
        ...
        st.error("Data processing failed. Please ensure a valid input is provided.")